var Bt=Object.defineProperty;var Rt=(a,t,n)=>t in a?Bt(a,t,{enumerable:!0,configurable:!0,writable:!0,value:n}):a[t]=n;var p=(a,t,n)=>Rt(a,typeof t!="symbol"?t+"":t,n);import{S as Ne,C as zt,M as je,P as Xe,V as z,a as He,b as Ke,c as It,W as Vt,L as Je,H as qt,d as _t,e as Ze,f as Gt,g as Ot,h as xe,i as I,D as Wt,R as Yt,U as Nt,N as Xt,j as Ht,k as Kt,A as Jt,O as Zt,T as Qt,l as eo,m as to,n as oo,F as ao,o as ro,B as io,p as Mt,G as so,q as no,r as lo,s as co,t as uo}from"./three.module-DXmDt1YS.js";import{v as e,h as Qe,p as Ge,a as ne,s as ho,b as E,c as Ce,d as po,r as Fe,e as vo,g as Oe,f as We,i as Pt,j as Ct,k as mo}from"./next-BbbdK-ql.js";import"./lenis-CH_BOYM6.js";const fo=`varying vec2 vUv;
void main() { vUv = position.xy * .5 + .5; gl_Position = vec4(position.xy, 0., 1.); }`,Ft="#include <tonemapping_pars_fragment>",W=(a=!1,t=0)=>new Vt(1,1,{type:qt,minFilter:Je,magFilter:Je,depthBuffer:a,stencilBuffer:!1,samples:t}),ie=(a,t)=>new It({vertexShader:fo,fragmentShader:a,uniforms:t,depthTest:!1,depthWrite:!1,toneMapped:!1});class go{constructor(t){p(this,"quadScene",new Ne);p(this,"quadCamera",new zt);p(this,"quad",new je(new Xe(2,2)));p(this,"color",W(!0,4));p(this,"mask",W(!0,4));p(this,"background",W());p(this,"flare",W());p(this,"velocity",W());p(this,"velocitySwap",W());p(this,"pressure",W());p(this,"pressureSwap",W());p(this,"divergence",W());p(this,"texel",new z(1,1));p(this,"pointer",new z(-1,-1));p(this,"pointerFrom",new z(-1,-1));p(this,"impulse",new z);p(this,"pixel",new z(1,1));p(this,"aspect",{value:1});p(this,"dt",{value:1/60});p(this,"maskMaterial",new He({color:16777215,toneMapped:!1}));p(this,"advect");p(this,"diverge");p(this,"solve");p(this,"project");p(this,"star");p(this,"composite");p(this,"energy",0);p(this,"stale",!0);p(this,"backgroundDirty",!0);p(this,"cadence",0);p(this,"flareAllowed",!0);p(this,"disposed",!1);p(this,"width",1);p(this,"height",1);this.renderer=t,this.quad.material.dispose(),this.color.samples=this.mask.samples=Math.min(4,t.capabilities.maxSamples),this.quad.frustumCulled=!1,this.quadScene.add(this.quad),this.advect=ie(`
...
#include <opaque_fragment>`)},q=()=>`${x}:gxc-sky-backdrop-v9-indexed-stars`;return a.onBeforeCompile=j,a.customProgramCacheKey=q,a.needsUpdate=!0,{setProjectAtlas(b){M||(S.uProjectAtlas.value=b,b||(S.uProjectCount.value=0))},updateProjects(b){if(M)return;const T=b.points.filter(D=>Number.isInteger(Dt[D.slug])),A=S.uProjectAtlas.value?Math.min(T.length,w.capacity):0;S.uProjectCount.value=A;for(let D=0;D<A;D++){const y=T[D],P=y.angle*Math.PI/180;f[D].set(y.x,y.y,y.size,Math.max(0,Math.min(1,y.opacity))),s[D].set(Math.cos(P),Math.sin(P),y.tailLength,Math.max(0,Math.min(1,y.labelOpacity))),g[D]=Dt[y.slug]}},updateTwinkles(b){var D;const T=S.uPhotoStarCount.value;S.uPhotoStarCount.value=M?0:Math.min(b.length,E.capacity);let A=T!==S.uPhotoStarCount.value;for(let y=0;y<S.uPhotoStarCount.value;y++){const P=b[y],F=d[y];(F.x!==P.u||F.y!==P.v||F.z!==P.radiusPx)&&(A=!0),F.set(P.u,P.v,P.radiusPx,P.amplitude),u[y].fromArray(((D=P.overlay)==null?void 0:D.color)??[1,1,1])}!M&&A&&(yo(C,b.slice(0,S.uPhotoStarCount.value),Ce.source.width,Ce.source.height,E.supportSigma),m.needsUpdate=!0)},update(b,T,A,D){if(M)return;i.set(Math.max(T,1),Math.max(A,1),D);const y=Math.min(b.streaks.length,se);S.uSkyCount.value=y;for(let P=0;P<y;P++){const F=b.streaks[P],U=F.angle*Math.PI/180;c[P].set(F.x,F.y,F.length,Math.max(0,F.opacity)),v[P].set(Math.cos(U),Math.sin(U),F.width,0)}},dispose(){M||(M=!0,S.uSkyCount.value=0,S.uProjectCount.value=0,S.uProjectAtlas.value=null,S.uPhotoStarCount.value=0,m.dispose(),a.onBeforeCompile===j&&(a.onBeforeCompile=t),a.customProgramCacheKey===q&&(a.customProgramCacheKey=n),a.needsUpdate=!0)}}}async function Po(a,t,n,x){var C;const i=new Kt({alpha:!0,antialias:!0,powerPreference:"high-performance",preserveDrawingBuffer:!1}),c=[];let v=!1,d=!1;const u=()=>{if(!v){v=!0;for(const m of c.reverse())m();i.dispose(),i.domElement.remove()}};try{i.setClearColor(592396,1),i.outputColorSpace=Ze,i.toneMapping=Jt,i.toneMappingExposure=e.lighting.exposure,i.debug.onShaderError=()=>{throw new Error("Glass shader could not compile")},i.domElement.setAttribute("aria-hidden","true"),a.appendChild(i.domElement);const m=new Ne,f=new Zt(-20,20,10,-10,.1,150);f.position.set(0,0,40);const s=a.closest(".gxc-hero")??a;let g,S,M,j;const q=new Promise((r,h)=>{M=r,j=h});c.push(po(s,r=>{g=r,M(r),S==null||S(r)},()=>j(new Error("Background photograph could not load"))));const[b,T]=await Promise.allSettled([fetch("/v-next/galaxci-inflated-mesh.json").then(r=>{if(!r.ok)throw new Error("Wordmark could not load");return r.json()}),q]);if(b.status==="rejected")throw b.reason;if(T.status==="rejected")throw T.reason;const A=b.value;g=g??T.value;const D=r=>{const h=new Qt(r.image);return h.colorSpace=Ze,h.needsUpdate=!0,h};let y=D(g);c.push(()=>y.dispose());const P=new Xe(1,1),F=new He({map:y,color:e.lighting.backdropTint,toneMapped:!1});c.push(()=>P.dispose(),()=>F.dispose());const U=new je(P,F);U.position.z=-6,m.add(U);const L=wo(F);let _=-1,Ue=-1,ye=-1,K,et=()=>{};new URLSearchParams(location.search).has("no-project-atlas")?a.dataset.projectAtlas="disabled":xo().then(r=>{if(v||d){r.dispose();return}K=r,L.setProjectAtlas(r),a.dataset.projectAtlas="ready",et()}).catch(()=>{v||(a.dataset.projectAtlas="failed")}),c.push(()=>{L.dispose(),K==null||K.dispose(),delete s.dataset.skyReady,delete s.dataset.projectSkyReady});const Ae=new Ne;Ae.background=new Ke(1118742);const tt=[],ot=new eo(i);try{for(const h of e.lighting.panels){const k=new He({color:new Ke(h.color).multiplyScalar(h.strength),side:to}),l=new je(new Xe(...h.size),k);l.position.set(h.position[0],h.position[1],h.position[2]),l.lookAt(0,0,0),Ae.add(l),tt.push(l)}const r=ot.fromScene(Ae,.06);c.push(()=>r.dispose()),m.environment=r.texture}finally{for(const r of tt)r.geometry.dispose(),r.material.dispose();ot.dispose()}const Le=new oo({color:e.glass.tint,metalness:0,roughness:e.glass.roughness,transmission:1,thickness:e.glass.thickness,ior:e.glass.ior,dispersion:e.glass.dispersion,envMapIntensity:e.glass.environment,clearcoat:e.glass.clearcoat,clearcoatRoughness:.04,attenuationColor:e.glass.attenuation,attenuationDistance:50,side:ao});c.push(()=>Le.dispose()),Le.onBeforeCompile=r=>{r.uniforms.gxcScatterStrength={value:new URLSearchParams(location.search).has("no-scattering")?0:e.scattering.strength},r.fragmentShader=`uniform float gxcScatterStrength;
`+r.fragmentShader;const h=ro.transmission_fragment.replace("transmitted.rgb, material.transmission",`transmitted.rgb * ${e.glass.starExposure.toFixed(1)}, material.transmission`);r.fragmentShader=r.fragmentShader.replace("#include <transmission_fragment>",h+`
        ...
        #endif
      `)};const G=new io;c.push(()=>G.dispose()),G.setAttribute("position",new Mt(A.positions,3)),G.setAttribute("normal",new Mt(A.normals,3)),G.setIndex(A.indices),G.computeBoundingBox();const at=G.boundingBox.getSize(new I),Tt=G.boundingBox.getCenter(new I),we=new je(G,Le),J=new so;J.add(we),m.add(J);const rt=new no(15397631,e.lighting.point,90,2);m.add(rt);const it=new lo(16777215,e.lighting.fill);it.position.set(-5,9,6),m.add(it);const $e=new URLSearchParams(location.search),Se=matchMedia("(hover: hover) and (pointer: fine)"),Ut=$e.has("no-postfx"),Ee=$e.has("no-fluid"),st=$e.has("no-flare");let o,le=!i.extensions.has("EXT_color_buffer_float"),be=!n,ke=!0,Z=!0,ce=!1,ue=0,de=0,Q=e.rimLight.angle,Me=e.rimLight.angle,he=.07,pe=-.07,ee=0,te=0,Y=0,N=0,Be=0,nt=0,Pe=0,Re=-1,At=0;et=()=>{ye=-1,Z=!0,o==null||o.invalidateBackground(),Fe()};const lt=new z,oe=new z,ct=new z,ve=new I,ae=new I,ut=new I,dt=new I,ht=new co,pt=new z,vt=new uo(new I(0,0,1),0),ze=((C=a.parentElement)==null?void 0:C.parentElement)??a,X=(r=!1)=>{ue=0,de=0,Me=e.rimLight.angle,ce=!1,Re=-1,r&&(he=.07,pe=-.07,Q=e.rimLight.angle,ee=0,te=0,o==null||o.reset()),Z=!0,Fe()},Ie=()=>{if(!(be&&Se.matches&&!Ut&&!(Ee&&st)&&!le))o==null||o.dispose(),o=void 0;else if(!o)try{o=new go(i),o.setFlare(!st),o.setSize(Y||1,N||1,Be||1)}catch{o==null||o.dispose(),o=void 0,le=!0}i.domElement.dataset.postfx=o?"enabled":le?"unsupported":"disabled"};c.push(()=>o==null?void 0:o.dispose());const Ve=()=>{if(v||d)return;const r=a.getBoundingClientRect(),h=t.getBoundingClientRect(),k=r.width,l=r.height;if(!k||!l)return;Pe=r.left,nt=r.top+window.scrollY;const $=Math.min(devicePixelRatio,k<700?e.glass.mobileDpr:e.glass.maxDpr);(k!==Y||l!==N||$!==Be)&&(i.setPixelRatio($),i.setSize(k,l,!1),Y=k,N=l,Be=$,o==null||o.setSize(k,l,$));const B=26,R=B*k/l;f.left=-R/2,f.right=R/2,f.top=B/2,f.bottom=-B/2,f.updateProjectionMatrix();const re=k<700?22:80;J.scale.setScalar(Math.min((k-re*2)/at.x,h.height*.88/at.y)*B/l),J.position.y=(l/2-(h.top-r.top+h.height/2))*B/l;const O=e.cameraMotion.overscan;U.scale.set(R*O,B*O,1);const H=vo(k,l,g.width,g.height);y.repeat.set(k/H.width,l/H.height),y.repeat.multiplyScalar(O),y.offset.set((1-y.repeat.x)/2,(1-y.repeat.y)/2),L.update(Oe(s),k,l,O),o==null||o.invalidateBackground(),qe(),Lt(),Z=!0,Fe()},qe=()=>{J.rotation.set(he,pe,-.018),J.updateMatrixWorld(!0),ve.copy(Tt).applyMatrix4(we.matrixWorld),rt.position.set(ve.x+Math.cos(Q)*e.rimLight.radius,ve.y+Math.sin(Q)*e.rimLight.radius,e.rimLight.z),f.position.set(ee,te,40),ut.set(ee*e.cameraMotion.lookAtFactor,te*e.cameraMotion.lookAtFactor,0),f.lookAt(ut),f.updateMatrixWorld(),f.getWorldDirection(dt),U.position.copy(f.position).addScaledVector(dt,46),U.quaternion.copy(f.quaternion),U.updateMatrixWorld(!0)},Lt=()=>{const r=G.boundingBox,h=new I;let k=1/0,l=1/0,$=-1/0,B=-1/0;for(const re of[r.min.x,r.max.x])for(const O of[r.min.y,r.max.y])for(const H of[r.min.z,r.max.z]){h.set(re,O,H).applyMatrix4(we.matrixWorld).project(f);const fe=(h.x+1)*Y/2,ge=(1-h.y)*N/2;k=Math.min(k,fe),$=Math.max($,fe),l=Math.min(l,ge),B=Math.max(B,ge)}const R=18;a.dataset.wordRect=JSON.stringify({left:k-R,top:l-R,width:$-k+R*2,height:B-l+R*2})};S=r=>{if(v||d)return;const h=y;y=D(r),F.map=y,L.updateTwinkles(r.fallback?[]:We(s).points),i.domElement.dataset.photoSource=r.url,Ve(),h.dispose()},i.domElement.dataset.photoSource=g.url;const mt=r=>{const h=Oe(s),k=We(s),l=Pt(s);if(l.revision!==ye&&(ye=l.revision,L.updateProjects(l),o==null||o.invalidateBackground()),k.revision!==Ue&&(Ue=k.revision,L.updateTwinkles(g!=null&&g.fallback?[]:k.points),o==null||o.invalidateBackground()),h.revision!==_&&(_=h.revision,L.update(h,Y,N,e.cameraMotion.overscan),o==null||o.invalidateBackground()),o)try{o.render(m,f,we,U,r,be&&!Ee)}catch{o.dispose(),o=void 0,le=!0,i.domElement.dataset.postfx="failed",i.setRenderTarget(null),i.render(m,f)}else i.render(m,f);i.domElement.dataset.frames=String(++At),s.dataset.skyReady="true",K&&l.points.length?s.dataset.projectSkyReady="true":delete s.dataset.projectSkyReady,i.domElement.dataset.skyCount=String(h.streaks.length),i.domElement.dataset.twinkleCount=String(k.points.length),i.domElement.dataset.projectCount=String(K?l.points.length:0),i.domElement.dataset.fluid=o!=null&&o.active?"active":"rest",Z=!1},ft=r=>{r.preventDefault(),d=!0,ke=!1,a.dataset.failed="context-lost",x(),u()},gt=()=>X(),xt=()=>{document.hidden?X(!0):(Z=!0,Fe())},yt=()=>{X(!0),Ie()},wt=()=>X();i.domElement.addEventListener("webglcontextlost",ft),ze.addEventListener("pointerleave",wt),window.addEventListener("blur",gt),document.addEventListener("visibilitychange",xt),Se.addEventListener("change",yt),c.push(()=>{i.domElement.removeEventListener("webglcontextlost",ft),ze.removeEventListener("pointerleave",wt),window.removeEventListener("blur",gt),document.removeEventListener("visibilitychange",xt),Se.removeEventListener("change",yt)});const _e=new ResizeObserver(Ve);_e.observe(a),_e.observe(t),c.push(()=>_e.disconnect());const St=new IntersectionObserver(r=>{ke=r[0].isIntersecting,X(!0)});if(St.observe(ze),c.push(()=>St.disconnect()),Ve(),Ie(),qe(),await i.compileAsync(m,f),o)try{await o.warm()}catch{o.dispose(),o=void 0,le=!0,i.domElement.dataset.postfx="failed"}if(v||d)throw new Error("Glass context unavailable");mt(1/60);let me=!1;const $t=Ct((r,h)=>{if(!ke||v||d||document.hidden)return!1;const k=mo(),l=k.pointer,$=nt-k.scrollY;be&&Se.matches&&l.kind!=="touch"&&l.inside&&l.x>=Pe&&l.x<=Pe+Y&&l.y>=$&&l.y<=$+N?(oe.set((l.x-Pe)/Y,1-(l.y-$)/N),ue=oe.x*2-1,de=1-oe.y*2,pt.set(ue,-de),ht.setFromCamera(pt,f),vt.constant=-ve.z,ht.ray.intersectPlane(vt,ae)&&(ae.sub(ve),Math.hypot(ae.x,ae.y)>26*e.rimLight.centerDeadZone&&(Me=Math.atan2(ae.y,ae.x))),l.lastMoved!==Re&&(ce&&o&&!Ee&&(ct.copy(oe).sub(lt),o.push(oe,ct)),lt.copy(oe),Re=l.lastMoved),ce=!0):ce&&X();const R=.07+de*e.pointer.rotationX,re=-.07+ue*e.pointer.rotationY,O=ue*e.cameraMotion.offsetX,H=-de*e.cameraMotion.offsetY,fe=1-Math.exp(-e.pointer.damping*h),ge=1-Math.exp(-(ce?e.cameraMotion.damping:e.cameraMotion.leaveDamping)*h);he+=(R-he)*fe,pe+=(re-pe)*fe,ee+=(O-ee)*ge,te+=(H-te)*ge;const bt=Math.atan2(Math.sin(Me-Q),Math.cos(Me-Q));Q+=bt*(1-Math.exp(-e.rimLight.damping*h));const kt=Math.abs(R-he)+Math.abs(re-pe)+Math.abs(bt)+Math.abs(O-ee)+Math.abs(H-te)>2e-4;return(Z||kt||o!=null&&o.active)&&(qe(),me=!0),kt||!!(o!=null&&o.active)},"update"),Et=Ct((r,h)=>!ke||v||d||document.hidden?(me=!1,!1):((Oe(s).revision!==_||We(s).revision!==Ue||Pt(s).revision!==ye)&&(me=!0),me&&(mt(h),me=!1),!!(o!=null&&o.active)),"render");return c.push($t,Et),{setMotion(r){v||d||(be=r,X(!0),Ie())},dispose:u}}catch(m){throw u(),m}}export{Po as mountGlass};
