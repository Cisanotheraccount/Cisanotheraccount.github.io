...
import { applyPhotographyRelease, catalogOutputPath, overlayPhotographyRelease, releaseManifestPath, validatePhotographyCatalog } from './photography-release.mjs';
...
try {
  ...
  assert.equal(result.preservedFiles, 5); assert.equal(await readFile(path.join(output, 'galaxci/index.html'), 'utf8'), 'frozen-two');
  ...
  console.log('Photography release checks passed: catalog completeness, HDR JPEG receipts, traversal guards, Git baseline rejection, and non-photography baseline preservation.');
