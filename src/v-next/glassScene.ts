...
export async function mountGlass(host: HTMLElement, area: HTMLElement, disabled: boolean, onLost: () => void): Promise<GlassScene> {
  ...
  try {
    ...
    let lastPointerTime = -1, frames = 0;
    invalidateProjects = () => {
    ...
    const reset = (immediate = false) => {
      ...
      if (immediate) { rx = .07; ry = -.07; angle = visual.rimLight.angle; cameraX = 0; cameraY = 0; post?.reset(); }
      dirty = true; requestFrame();
    };
    const updatePost = () => {
      const wanted = enabled && finePointer.matches && !noPost && !(noFluid && noFlare) && !postFailed;
      if (!wanted) { post?.dispose(); post = undefined; }
      else if (!post) {
        try { post = new HeroPost(renderer); post.setFlare(!noFlare); post.setSize(lastWidth || 1, lastHeight || 1, lastDpr || 1); }
        catch { post?.dispose(); post = undefined; postFailed = true; }
      }
      renderer.domElement.dataset.postfx = post ? 'enabled' : postFailed ? 'unsupported' : 'disabled';
    };
    ...
    const publishWordRect = () => {
      ...
      const margin = 18;
      host.dataset.wordRect = JSON.stringify({ left: minX - margin, top: minY - margin, width: maxX - minX + margin * 2, height: maxY - minY + margin * 2 });
    };
    replacePhoto = asset => {
      if (disposed || contextLost) return;
    ...
    const offUpdate = subscribeFrame((_time, dt) => {
      ...
      if (canPoint) {
        ...
        pointerX = uv.x * 2 - 1; pointerY = 1 - uv.y * 2;
      ...
      if (dirty || moving || post?.active) { applyPose(); pendingRender = true; }
