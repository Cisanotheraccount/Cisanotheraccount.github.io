...
const createState = (): SceneState => ({
  ready: false, tier: 'baseline', phase: 'warming', reason: 'Measuring browser cadence at full quality',
  retries: 0, lastDowngrade: -Infinity, triedHigh: false, trialFrom: 'baseline',
  goodTrialWindows: 0, badWindows: 0, badDuration: 0, healthyDuration: 0, stats: null,
});
...
export class PerformanceGovernor {
  ...
  /** Resize/DPR/context recovery requalifies full quality, preserving per-session retry history. */
  resetMeasurements(nowMs: number, reason = 'Viewport changed; rechecking full-quality performance') {
    ...
    for (const state of Object.values(this.states)) {
      ...
      state.healthyDuration = 0; state.stats = null;
    }
    this.resetSamples(nowMs);
  }
  ...
  getSnapshot(scene: PerformanceScene | null = this.scene): PerformanceSnapshot {
    ...
    return {
      scene, targetFps: this.targetFps(scene), observedHz: this.observedHz,
  ...
  private targetFps(scene: PerformanceScene | null = this.scene) {
    const state = scene ? this.states[scene] : this.inactiveState;
    const ceiling = state.tier === 'high' ? policy.highCeiling
  ...
  private finishWindow(nowMs: number) {
    ...
    if (state.tier === 'high' && state.badWindows >= policy.highBadWindows) {
      ...
      this.downgrade('low', nowMs, 'Five seconds over budget; preserving quality at the 30fps ceiling');
  ...
  private maybeUpgrade(nowMs: number) {
    ...
    if (!state.ready || this.warmStart === null || nowMs - this.warmStart < policy.warmupMs
      || this.observedHz === null || state.phase === 'trial' || state.tier === 'static' || state.tier === 'high') return;
    if (state.phase === 'warming') {
      state.phase = 'stable'; state.reason = 'Following the browser cadence at full quality';
    }
    ...
    state.reason = 'Testing a higher frame rate with the current full-quality scene';
  ...
  private downgrade(tier: Tier, nowMs: number, reason: string) {
    ...
    state.healthyDuration = 0; state.goodTrialWindows = 0;
    this.nextRender = null; this.lastRendered = null; this.clearWindow();
  }
}
