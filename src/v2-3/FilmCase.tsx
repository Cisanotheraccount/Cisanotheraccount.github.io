  ...
  return <div className="fc-case">
    <header className="fc-hero">
      ...
      <div className="fc-hero-copy">
        ...
        <div className="fc-hero-bottom"><p>{item.summary}</p><button className="fc-watch" onClick={watch} aria-controls="fc-screening"><Play size={17} aria-hidden="true"/>Watch the film<ArrowDown size={17} aria-hidden="true"/></button></div>
      </div>
    </header>

    <div className="fc-body">
      <section ref={screening} id="fc-screening" className="fc-screening" tabIndex={-1} aria-labelledby="fc-film-title">
        <div className="fc-section-heading"><span className="gxc-mono">01 / THE FILM</span><h3 id="fc-film-title">{item.video?.title}</h3><span className="fc-inline-note">Watch here, in the page.</span></div>
        <ProjectVideo project={item} active={active} requested={requested} onRequest={watch}/>
        <p className="fc-network-note">Playback requires access to YouTube. If the player cannot connect, you can still explore this project below.</p>
      </section>

      <section className="fc-overview" aria-label="Project overview">
        <aside><span className="gxc-mono">02 / ABOUT THE WORK</span><ul>{item.tags.map(tag => <li key={tag}>{tag}</li>)}</ul>{item.role && <p>Role — {item.role}</p>}{item.period && <p>{item.period}</p>}</aside>
      ...
      {studies.length > 0 && <section className="fc-stills" aria-labelledby="fc-stills-title"><div className="fc-section-heading"><span className="gxc-mono">03 / SELECTED FRAMES</span><h3 id="fc-stills-title">Inside the world.</h3></div><div className="fc-stills-grid">{studies.map(study => <div key={study.image}>{renderStudy(study)}</div>)}</div></section>}
    </div>
  </div>;
}
