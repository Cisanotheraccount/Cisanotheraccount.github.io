...
export class HeroPost {
  ...
  constructor(private readonly renderer: THREE.WebGLRenderer, private readonly nativePhoto = true) {
    ...
    this.composite = shader(`
      ...
      void main() {
        ...
        if(all(greaterThanEqual(vUv,uStaticGlassRect.xy)) && all(lessThanEqual(vUv,uStaticGlassRect.zw))) d=vec2(0.);
  ...
  setStaticGlassRect(rect?: { left: number; top: number; width: number; height: number }, width = 1, height = 1) {
    const value = this.composite.uniforms.uStaticGlassRect.value as THREE.Vector4;
    if (!rect) { value.set(2, 2, 2, 2); return; }
    const padding = 6;
    value.set((rect.left - padding) / width, 1 - (rect.top + rect.height + padding) / height,
      (rect.left + rect.width + padding) / width, 1 - (rect.top - padding) / height);
  }
